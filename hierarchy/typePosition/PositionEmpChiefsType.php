...
use frontend\components\sampling\conditionquery\hierarchy\decorator\{ChiefsDecorator, EmpDecorator};
use frontend\components\sampling\conditionquery\hierarchy\PositionComponent;

class PositionEmpChiefsType extends BaseHierarchyType
{
    public function addCondition(array $cnd = [])
    {
        $positionComponent = new PositionComponent($this->query, $cnd);
        $employDecorator = new EmpDecorator($positionComponent);

        return new ChiefsDecorator($employDecorator);
    }
}